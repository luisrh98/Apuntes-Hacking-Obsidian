---------------------
# Gestores de contenidos (CMS)

| [[Joomla]]   | [[Drupal]]     | [[Magento]]  | [[Wordpress]] |
| ------------ | -------------- | ------------ | ------------- |
| [[Joomscan]] | [[Droopescan]] | [[Magescan]] | [[Wpscan]]    |

------------------------------------------
# **Herramientas de Reconocimiento**
## (Escaneo de Puertos y Servicios)

- [[Nmap]]

------------
## (Fuzzing de directorios, subdominios, DNS...)

| [[Gobuster]] | [[Wfuzz]] | [[Ffuf]]        |
| ------------ | --------- | --------------- |
| Más completa |           | Más rápida (Go) |

--------------------------------
## Enumeración de Tecnologías Web

- [[Wappalyzer]]
- [[Whatweb]]

## Enumeración de Sistemas Operativos

| Linux                       | Windows     | Android | Mac |
| --------------------------- | ----------- | ------- | --- |
| [[Linux-smart-enumeration]] | [[Winpeas]] |         |     |
| [[Linpeas]]                 |             |         |     |

-------------------------
# Herramientas de Fuerza Bruta

- [[Hydra]]

--------------------------

# OSINT

## Base de datos Filtradas

- [[DeHashed]]

## Correos Electrónicos y RRSS

- [[Clearbit Connect]]
- [[Hunter.io]]
- [[Intelligence X]]
- [[Phonebook.cz]]

## Imágenes

- [[PimEyes]]

--------
# Herramientas de Explotación

- [[Msfvenom]]
- [[Metasploit]]
- [[Sqlmap]]

---
# Pivoting

- [[Pivoting – Chisel, Socat y Netsh Portproxy]]

---
# Buffer-Overflow

- [[Buffer Overflow en Linux usando GDB y herramientas de Metasploit (sin framework)]]
- [[Buffer Overflow en Windows con Immunity Debugger + Mona]]
- [[mona.py]]
---
# Otras Herramientas

### Botnet
- [Creación de botnets](https://github.com/epsylon/ufonet)
